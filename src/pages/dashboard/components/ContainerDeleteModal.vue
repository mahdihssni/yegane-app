...
import { removeContainer } from '../../../api/container.ts';
...
const onDeleteButtonClicked = async () => {
    try {
        loading.value = true;
        await removeContainer(props.id);
        emits('close');
    } finally {
        loading.value = false;
    }
...
<template>
    <Modal :title="`Remove '${props.title}' container`" :show="props.show">
        Are you sure want to remove the {{ props.title }} container?
